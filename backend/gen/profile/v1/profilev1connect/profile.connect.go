...
import (
	...
	context "context"
	errors "errors"
	v1 "github.com/chejholloway/conduit-backend/gen/profile/v1"
	http "net/http"
	strings "strings"
)
...
const (
	...
	// ProfileServiceFollowProcedure is the fully-qualified name of the ProfileService's Follow RPC.
	ProfileServiceFollowProcedure = "/profile.v1.ProfileService/Follow"
	// ProfileServiceUnfollowProcedure is the fully-qualified name of the ProfileService's Unfollow RPC.
	ProfileServiceUnfollowProcedure = "/profile.v1.ProfileService/Unfollow"
)
...
func NewProfileServiceClient(httpClient connect.HTTPClient, baseURL string, opts ...connect.ClientOption) ProfileServiceClient {
	...
	return &profileServiceClient{
		...
		follow: connect.NewClient[v1.FollowRequest, v1.FollowResponse](
			httpClient,
			baseURL+ProfileServiceFollowProcedure,
			connect.WithSchema(profileServiceMethods.ByName("Follow")),
			connect.WithClientOptions(opts...),
		),
		unfollow: connect.NewClient[v1.UnfollowRequest, v1.UnfollowResponse](
			httpClient,
			baseURL+ProfileServiceUnfollowProcedure,
			connect.WithSchema(profileServiceMethods.ByName("Unfollow")),
			connect.WithClientOptions(opts...),
		),
	}
}
...
// Follow calls profile.v1.ProfileService.Follow.
func (c *profileServiceClient) Follow(ctx context.Context, req *connect.Request[v1.FollowRequest]) (*connect.Response[v1.FollowResponse], error) {
	return c.follow.CallUnary(ctx, req)
}

// Unfollow calls profile.v1.ProfileService.Unfollow.
func (c *profileServiceClient) Unfollow(ctx context.Context, req *connect.Request[v1.UnfollowRequest]) (*connect.Response[v1.UnfollowResponse], error) {
...
func NewProfileServiceHandler(svc ProfileServiceHandler, opts ...connect.HandlerOption) (string, http.Handler) {
	...
	profileServiceFollowHandler := connect.NewUnaryHandler(
		ProfileServiceFollowProcedure,
		svc.Follow,
		connect.WithSchema(profileServiceMethods.ByName("Follow")),
		connect.WithHandlerOptions(opts...),
	)
	profileServiceUnfollowHandler := connect.NewUnaryHandler(
		ProfileServiceUnfollowProcedure,
		svc.Unfollow,
		connect.WithSchema(profileServiceMethods.ByName("Unfollow")),
		connect.WithHandlerOptions(opts...),
	)
	return "/profile.v1.ProfileService/", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case ProfileServiceFollowProcedure:
			profileServiceFollowHandler.ServeHTTP(w, r)
		case ProfileServiceUnfollowProcedure:
			profileServiceUnfollowHandler.ServeHTTP(w, r)
		default:
			http.NotFound(w, r)
		}
	})
...
func (UnimplementedProfileServiceHandler) Follow(context.Context, *connect.Request[v1.FollowRequest]) (*connect.Response[v1.FollowResponse], error) {
	return nil, connect.NewError(connect.CodeUnimplemented, errors.New("profile.v1.ProfileService.Follow is not implemented"))
}

func (UnimplementedProfileServiceHandler) Unfollow(context.Context, *connect.Request[v1.UnfollowRequest]) (*connect.Response[v1.UnfollowResponse], error) {
	return nil, connect.NewError(connect.CodeUnimplemented, errors.New("profile.v1.ProfileService.Unfollow is not implemented"))
}
